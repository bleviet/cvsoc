...
# for regmap_conformance, driven entirely over JTAG-to-Avalon-MM -- no Nios
# II firmware required (docs/hardware-conformance-test-plan.md, "Component
# 3", System Console host / ipcraft-vscode issue #36 preparation).
#
# Runs the same check sequence already proven on GHDL by
# tb/regmap_conformance_test.py and on the Nios II host by
# software/app/main.c. Prints PASS/FAIL per check, a final sentinel, and
# exits nonzero on any failure so a CI runner can gate on it.
#
# Usage:
#   system-console --cli --script=conformance_sysconsole.tcl
#
# Or from the Makefile:
#   make debug-conformance
# ---------------------------------------------------------------------------

set REGMAP_BASE 0x00010000
...
puts "@@DONE"
# NOTE: `exit` is not a valid command in System Console's --cli REPL (it
# raises "invalid command name \"exit\""). The Makefile's conformance-sysconsole
# target greps this script's stdout for the @@RESULT sentinel instead of
# relying on a process exit code.
