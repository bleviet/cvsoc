/*
 * main.c — Nios V/m GDB debugging demo for the DE10-Nano board.
 *
 * This application is purpose-built for demonstrating source-level GDB
 * debugging of a Nios V/m soft-core via the Ashling RiscFree GDB server.
 *
 * Debugging concepts demonstrated:
 *   1. Breakpoints               — 'break set_led'
 ...
 *     → Nios V/m platform_irq_rx IRQ 1 → button_isr()
 *
 ...
 *
 *   Setting a breakpoint here lets you inspect the pattern argument and the
 *   LED PIO register before/after the write:
 *     (gdb) break set_led
 *     (gdb) commands
 *       print pattern
 *       x/1xw LED_PIO_BASE
